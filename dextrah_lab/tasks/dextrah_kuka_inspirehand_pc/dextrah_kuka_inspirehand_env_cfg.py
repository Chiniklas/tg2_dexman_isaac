...
import warp as wp
import math
from dextrah_lab.assets.kuka_inspirehand_modified.kuka_inspirehand import KUKA_INSPIREHAND_CFG
...
class DextrahKukaInspirehandEnvCfg(DirectRLEnvCfg):
    ...
    # robot
    robot_cfg: ArticulationCfg = KUKA_INSPIREHAND_CFG.replace(prim_path="/World/envs/env_.*/Robot").replace(
    ...
    hand_body_pc_exclude_prefixes = ("iiwa7",)
    ...
    # Point cloud overlay for object visualization.
    enable_object_pointcloud_overlay = True
    pointcloud_overlay_env_ids = None
    object_pointcloud_num_points = 1000
    object_pointcloud_marker_size = 0.003
    object_pointcloud_color = (0.0, 0.8, 1.0)
    object_pointcloud_opacity = 0.9
    use_pointcloud_hand_object_dist = True
    hand_object_pointcloud_num_points = 200
    hand_object_pointcloud_dist_clip = None
    encode_hand_object_dist =  True # this is the flag to add point cloud data into observation
    ...
    # reward weights
    # phase 1: reaching
    hand_to_object_weight = 20.  # legacy exponential form (unused in PC distance reward)
    hand_to_object_sharpness = 8. # legacy exponential form (unused in PC distance reward)
    hand_object_palm_dist_weight = -10.0
    hand_object_finger_dist_weight = -1.0
    hand_object_hold_max_finger_dist = 0.3
    hand_object_hold_max_palm_dist = 0.06
    hand_object_hold_value = 2
    ...
    action_rate_penalty_weight = 0.01 # default 5e-4
    ...
    episode_length_gate_dist = 0.15 # default 0.25
