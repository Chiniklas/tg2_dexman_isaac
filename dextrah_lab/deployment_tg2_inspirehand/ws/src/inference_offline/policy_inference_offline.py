#!/usr/bin/env python3
"""Replay recorded trajectory files (.h5) by stepping the environment with saved actions.

python policy_inference_offline.py \
  --traj_file /path/to/data/traj_env_0_file_0.h5 \
  --num_envs 1 \
  --task dextrah_tg2_inspirehand \
  --objects_dir /home/chizhang/projects/dextrah/tg2_dexman_isaac/dextrah_lab/assets/_single_object \
  --enable_cameras \
...
parser = argparse.ArgumentParser(description="Replay offline trajectory files in IsaacLab.")
...
def _resolve_traj_file() -> str:
    path = Path(args_cli.traj_file).expanduser().resolve()
    if not path.exists():
...
def _replay_one(env: gym.Env, path: str) -> TrajStats:
...
def main(env_cfg, _agent_cfg: dict) -> None:
    ...
    while simulation_app.is_running():
        replay_count += 1
        stats = _replay_one(env, traj_path)
    ...
    last_stats = all_stats[-1] if all_stats else TrajStats(path=traj_path, replay_steps=0, mean_reward=0.0, ever_lift_success_per_env=[])
