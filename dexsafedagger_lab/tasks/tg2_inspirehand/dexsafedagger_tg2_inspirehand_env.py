...
from .dexsafedagger_tg2_inspirehand_utils import (
    assert_equals,
    scale,
...
# ADR imports
from .dexsafedagger_adr import DexSafeDaggerADR

from .dexsafedagger_tg2_inspirehand_constants import (
    NUM_XYZ,
    NUM_RPY,
    NUM_QUAT,
    NUM_HAND_PCA,
...
from dexsafedagger_lab.utils.robot_frame_origins_taskmap import RobotFrameOriginsTaskMap
...
class DexSafeDaggerTG2InspirehandEnv(DirectRLEnv):
    cfg: DexSafeDaggerTG2InspirehandEnvCfg

    def __init__(self, cfg: DexSafeDaggerTG2InspirehandEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # Set up ADR
        self.dexsafedagger_adr =\
            DexSafeDaggerADR(self.event_manager, self.cfg.adr_cfg_dict, self.cfg.adr_custom_cfg_dict)
        ...
        self.dexsafedagger_adr.set_num_increments(self.cfg.starting_adr_increments)
...
#                torch.ones(self.num_envs, 1, device=self.device)
    ...
    def _apply_action(self) -> None:
        ...
        # Set velocity target
        vel_scale = self.dexsafedagger_adr.get_custom_param_value(
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        lift_weight = self.dexsafedagger_adr.get_custom_param_value("reward_weights", "lift_weight")
        (
            ...
            action_rate_penalty,
                ...
                self.dexsafedagger_adr.get_custom_param_value("reward_weights", "object_to_goal_sharpness"),
        ...
        self.extras["num_adr_increases"] = self.dexsafedagger_adr.num_increments()
...
#===================================================================================================================================
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        x_width_spawn = self.dexsafedagger_adr.get_custom_param_value("object_spawn", "x_width_spawn")
        y_width_spawn = self.dexsafedagger_adr.get_custom_param_value("object_spawn", "y_width_spawn")
        ...
        rotation = self.dexsafedagger_adr.get_custom_param_value("object_spawn", "rotation") 
        ...
        # Spawning robot
        joint_pos_noise = self.dexsafedagger_adr.get_custom_param_value("robot_spawn" ,"joint_pos_noise")
        joint_vel_noise = self.dexsafedagger_adr.get_custom_param_value("robot_spawn" ,"joint_vel_noise")
        ...
        self.object_pos_bias_width[env_ids, 0] =\
            self.dexsafedagger_adr.get_custom_param_value("object_state_noise", "object_pos_bias") *\
            torch.rand(num_ids, device=self.device)
        self.object_rot_bias_width[env_ids, 0] =\
            self.dexsafedagger_adr.get_custom_param_value("object_state_noise", "object_rot_bias") *\
            torch.rand(num_ids, device=self.device)
        ...
        self.object_pos_noise_width[env_ids, 0] =\
            self.dexsafedagger_adr.get_custom_param_value("object_state_noise", "object_pos_noise") *\
            torch.rand(num_ids, device=self.device)
        self.object_rot_noise_width[env_ids, 0] =\
            self.dexsafedagger_adr.get_custom_param_value("object_state_noise", "object_rot_noise") *\
            torch.rand(num_ids, device=self.device)

        # ROBOT NOISE---------------------------------------------------------------------------
        # Sample widths of uniform distribution controlling robot state bias
        self.robot_joint_pos_bias_width[env_ids, 0] =\
            self.dexsafedagger_adr.get_custom_param_value("robot_state_noise", "robot_joint_pos_bias") *\
            torch.rand(num_ids, device=self.device)
        self.robot_joint_vel_bias_width[env_ids, 0] =\
            self.dexsafedagger_adr.get_custom_param_value("robot_state_noise", "robot_joint_vel_bias") *\
            torch.rand(num_ids, device=self.device)
        ...
        self.robot_joint_pos_noise_width[env_ids, 0] =\
            self.dexsafedagger_adr.get_custom_param_value("robot_state_noise", "robot_joint_pos_noise") *\
            torch.rand(num_ids, device=self.device)
        self.robot_joint_vel_noise_width[env_ids, 0] =\
            self.dexsafedagger_adr.get_custom_param_value("robot_state_noise", "robot_joint_vel_noise") *\
            torch.rand(num_ids, device=self.device)
        ...
        if self.cfg.enable_adr:
            if self.step_since_last_dr_change >= self.cfg.min_steps_for_dr_change and \
                ...
                self.step_since_last_dr_change = 0
                self.dexsafedagger_adr.increase_ranges(increase_counter=True)
                ...
                self.local_adr_increment = torch.tensor(self.dexsafedagger_adr.num_increments(), device=self.device, dtype=torch.int64)
            else:
    ...
    def _compute_intermediate_values(self):
        ...
        self.robot_dof_vel_noisy *= self.dexsafedagger_adr.get_custom_param_value(
        ...
        self.hand_vel_noisy *= self.dexsafedagger_adr.get_custom_param_value(
            "observation_annealing"
            ,"coefficient"
        )
...
#                self.robot_dof_damping, # TODO: probably should scale these to be 0, 1
    ...
    def apply_object_wrench(self):
        ...
        max_linear_accel = self.dexsafedagger_adr.get_custom_param_value("object_wrench", "max_linear_accel")
